...
def main():
    ...
    if len(pareto_front) > 0:
        ...
        plt.savefig('reoptimization/pareto_front.png')
    ...
    if len(conv_hist) > 0:
        ...
        plt.tight_layout()
        plt.savefig('reoptimization/convergence.png')
    ...
    if len(pareto_front) > 0:
        ...
        log_results('reoptimization/simulation_log.txt', solver, final_pop)
...
def plot_layout(individual, title_prefix, filename):
    ...
    plt.savefig(f'reoptimization/{filename}')
...
def log_results(filename, solver, final_pop):
    ...
    with open(filename, 'w') as f:
        ...
        n_routers = getattr(solver, 'n_routers', N_ROUTERS)
